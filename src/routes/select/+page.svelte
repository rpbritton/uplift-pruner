	...
	<div class="flex-1 max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8 w-full">
		...
		<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 sm:mb-8">
			<div class="flex items-center gap-2 sm:gap-3">
				...
				<h1 class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">Uplift Pruner</h1>
			</div>
			{#if athlete}
				<div class="flex items-center gap-2 sm:gap-3 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 px-3 sm:px-4 py-2 self-stretch sm:self-auto">
					<div class="flex items-center gap-2 flex-1 sm:flex-initial min-w-0">
						{#if athlete.profile_medium}
							<img src={athlete.profile_medium} alt="Profile" class="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex-shrink-0" />
						{:else}
							<div class="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center flex-shrink-0">
								<User class="w-4 h-4 sm:w-5 sm:h-5 text-slate-500 dark:text-slate-400" />
							</div>
						{/if}
						<span class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">{athlete.firstname} {athlete.lastname}</span>
					</div>
		...
		<!-- Info box -->
		<div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6 mb-6 sm:mb-8">
		...
		<div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
			<div class="flex items-center justify-between mb-4 gap-2">
				<h2 class="text-lg sm:text-xl font-semibold text-slate-900 dark:text-slate-100">Select Activity</h2>
				<a
			...
			<ActivityInput on:activitySelected={handleActivitySelected} on:athleteLoaded={handleAthleteLoaded} />
